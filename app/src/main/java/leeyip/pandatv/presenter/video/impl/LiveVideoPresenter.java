...
public class LiveVideoPresenter extends LiveVideoContract.Presenter {
    ...
    @Override
    public void getPresenterRoomDetails(String room_id) {
        OkHttpClient okHttpClient = new OkHttpClient.Builder()
                .connectTimeout(20, TimeUnit.SECONDS)
                .readTimeout(20, TimeUnit.SECONDS)
                .build();
        okHttpClient.newCall(mModel.getModelRoomDetails(mContext, room_id)).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e("error", e.getMessage() + "---");
                L.e("错误信息:" + e.getMessage());
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    if (jsonObject.getInt("error") == 0) {
                        Gson gson = new Gson();
                        RoomDetailsInfo baseResponse = gson.fromJson(json, RoomDetailsInfo.class);
                        mView.getRoomDetails(baseResponse);
                    } else {
                        mView.showErrorWithStatus("获取数据失败!");
                    }
                } catch (JSONException e) {
